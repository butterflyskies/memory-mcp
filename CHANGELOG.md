...
- **Deterministic Markdown chunker** (#262 slice 2, ADR-0042): pure chunking module producing ordered, non-overlapping fact-level units whose spans tile the parent body exactly — split on structural boundaries (ATX headings, paragraphs, list items, fenced code blocks), coalescing adjacent units up to a bounded, deterministic boundary verified to fit the *actual* embedding tokenizer's budget (counted with special tokens, matching the truncation that silently clipped 18/191 measured memories; a verified fitting boundary, not a greedy longest-fitting run — real tokenizer counts are not monotone over prefixes), deterministic line-then-character splitting for oversized indivisible blocks, no overlap, and exactly one chunk for short memories. Boundary whitespace attaches to the preceding chunk (leading whitespace to the first), so concatenated chunk bodies reproduce the source byte for byte. The tokenizer and budget are constructor inputs behind a `TokenCounter` trait — the budget now comes from the model config's `max_position_embeddings` (also in the embedding engine, replacing the hard-coded 512) so a model swap (anticipated: ModernBERT, ~8192 context) carries its own budget; the chunker fingerprint (algorithm revision + tokenizer identity + budget) feeds the slice-3 staleness stamp so a swap forces a rebuild instead of mixing chunk vintages. Property-based tests cover ADR-0042 P2.1–P2.7 (determinism, span soundness, ordering, exact reconstruction, short-input degeneration, budget respect, prefix stability) over generated Markdown and arbitrary Unicode across three budgets (16/512/8192), against the real BGE tokenizer. Chunking is not yet wired into the catalog or indexes (slices 3+).
