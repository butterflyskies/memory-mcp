...
// These session options are no-ops for the candle backend. Candle handles its
// own threading and does not perform ONNX graph optimisation. Returning success
// allows fastembed (and other ort consumers) to use their normal session builder
// chain without modification.
unsafe extern "system" fn SetSessionGraphOptimizationLevel(_options: *mut OrtSessionOptions, _level: GraphOptimizationLevel) -> OrtStatusPtr {
	OrtStatusPtr::default()
}

unsafe extern "system" fn SetIntraOpNumThreads(_options: *mut OrtSessionOptions, _threads: ::core::ffi::c_int) -> OrtStatusPtr {
	OrtStatusPtr::default()
}

unsafe extern "system" fn SetInterOpNumThreads(_options: *mut OrtSessionOptions, _threads: ::core::ffi::c_int) -> OrtStatusPtr {
	OrtStatusPtr::default()
}
